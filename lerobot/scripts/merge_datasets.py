    ...
    for folder in source_folders:
        try:
            ...
            if os.path.exists(info_path):
                with open(info_path) as f:
                    ...
                    if "fps" in info:
                        ...
                        print(f" floder {folder} FPS={fps} (Dataset {folder} FPS={fps})")
            ...
            if parquet_path:
                ...
                if timestamp_cols:
                    print(
                        f"folder {folder} timestamps: {timestamp_cols} (Dataset {folder} contains timestamp columns: {timestamp_cols})"
                    )
                else:
                    issues.append(
                        f"folder :  {folder} (Warning: Dataset {folder} has no timestamp columns)"
                    )
            else:
                issues.append(
                    f"folder: {folder} (Warning: No parquet files found in dataset {folder})"
                )

        except Exception as e:
            issues.append(
                f"folder: {folder} e: {e} (Error: Failed to validate dataset {folder}: {e})"
            )
            print(f"e: {e} (Validation error: {e})")
            traceback.print_exc()

    # 检查FPS是否一致 (Check if FPS values are consistent)
    if len(set(fps_values)) > 1:
        issues.append(
            f"fps: : {fps_values} (Warning: Inconsistent FPS across datasets: {fps_values})"
        )
        ...
        if source_path:
            try:
                        ...
                        for _idx, value in enumerate(df[feature]):
                            if value is not None and isinstance(value, (list, np.ndarray)):
                                ...
                                if current_dim < max_dim:
                                    print(
                                        f"features {feature} dim {current_dim} max dim {max_dim}  (Padding {feature} from {current_dim} to {max_dim} dimensions)"
                                    )
                ...
                if "episode_index" in df.columns:
                    print(
                        f"episode index {df['episode_index'].iloc[0]} new {new_index} (Update episode_index from {df['episode_index'].iloc[0]} to {new_index})"
                    )
                ...
                if "index" in df.columns:
                    if episode_to_frame_index and new_index in episode_to_frame_index:
                        ...
                        print(
                            f"first index: {first_index} (Update index column, start value: {first_index} (using global cumulative frame count))"
                        )
                    else:
                        # (If no mapping provided, use current calculation as fallback)
                        first_index = new_index * len(df)
                        print(
                            f" first index: {first_index} (Update index column, start value: {first_index} (using episode index multiplied by length))"
                        )
                ...
                if "task_index" in df.columns and folder_task_mapping and old_folder in folder_task_mapping:
                    # (Get current task_index)
                    current_task_index = df["task_index"].iloc[0]

                    # (Check if there's a corresponding new index)
                    if current_task_index in folder_task_mapping[old_folder]:
                        new_task_index = folder_task_mapping[old_folder][current_task_index]
                        print(
                            f"current task index {current_task_index} new {new_task_index} (Update task_index from {current_task_index} to {new_task_index})"
                        )
                        df["task_index"] = new_task_index
                    else:
                        print(
                            f"current task index {current_task_index} (Warning: No mapping found for task_index {current_task_index})"
                        )
                ...
                total_copied += 1
                print(f"destination path: {dest_path} (Processed and saved: {dest_path})")

            except Exception as e:
                error_msg = f"source {source_path} e: {e} (Processing {source_path} failed: {e})"
                print(error_msg)
                traceback.print_exc()
                ...
                for file in files:
                    if file.endswith(".parquet") and f"episode_{old_index:06d}" in file:
                        try:
                                ...
                                if feature in df.columns:
                                    ...
                                    for _idx, value in enumerate(df[feature]):
                                        if value is not None and isinstance(value, (list, np.ndarray)):
                                            ...
                                            if current_dim < max_dim:
                                                print(
                                                    f"feature {feature} dim {current_dim} max {max_dim}  (Padding {feature} from {current_dim} to {max_dim} dimensions)"
                                                )
                                                # (Pad with zeros to target dimension)
                                                df[feature] = df[feature].apply(
                                                    lambda x: np.pad(
                            ...
                            if "episode_index" in df.columns:
                                print(
                                    f"episode index {df['episode_index'].iloc[0]} new index {new_index} (Update episode_index from {df['episode_index'].iloc[0]} to {new_index})"
                                )
                                df["episode_index"] = new_index
                            ...
                            if (
                                ...
                                if current_task_index in folder_task_mapping[old_folder]:
                                    new_task_index = folder_task_mapping[old_folder][current_task_index]
                                    print(
                                        f"current task index {current_task_index} new task {new_task_index} (Update task_index from {current_task_index} to {new_task_index})"
                                    )
                                    df["task_index"] = new_task_index
                                else:
                                    print(
                            ...
                            error_msg = f"source path {source_path} e: {e} (Processing {source_path} failed: {e})"
                            print(error_msg)
                            traceback.print_exc()
                            failed_files.append({"file": source_path, "reason": str(e), "episode": old_index})
                            total_failed += 1
            ...
            if not found:
                error_msg = f"old index {old_index} : {old_folder}"
                print(error_msg)
                failed_files.append(
                    {"file": f"episode_{old_index:06d}.parquet", "reason": "   ", "folder": old_folder}
                )
                total_failed += 1

    print(f"total copied {total_copied} total failed, {total_failed} ")
...
def pad_parquet_data(source_path, target_path, original_dim=14, target_dim=18):
    ...
    # (Ensure the target dimension is greater than the original dimension)
    new_df = df.copy()

    # (Check observation.state column)
    ...
    # (Save the new dataframe to target path)
...
def merge_datasets(
    ...
    # Initialize a mapping for episode indices to frame indices
    episode_to_frame_index = {}

    # Initialize a mapping for task descriptions to new indices
    task_desc_to_new_index = {}
    # Initialize a mapping for folder to task indices
    folder_task_mapping = {}
    ...
    # (Use simpler method to calculate total videos)
    total_videos = 0
    ...
    for folder in source_folders:
        try:
            ...
            if os.path.exists(folder_info_path):
                with open(folder_info_path) as f:
                    ...
                    if "total_videos" in folder_info:
                        ...
                        print(
                            f"{folder} info folder : {folder_videos} (Read video count from {folder}'s info.json: {folder_videos})"
                        )

            # Check dimensions of state vectors in this folder
            folder_dim = max_dim  # Default to max_dim
            ...
            for root, _dirs, files in os.walk(folder):
                for file in files:
                    if file.endswith(".parquet"):
                        ...
                        break
                if folder_dim != max_dim:  # Break if we found the dimension
                    break
            ...
            # Create a mapping for task_index to new task_index
            folder_task_mapping[folder] = {}

            # (Process tasks in each folder)
            for task in folder_tasks:
                task_desc = task["task"]
                old_index = task["task_index"]

                # Check if task_desc already exists in all_unique_tasks
                if task_desc not in task_desc_to_new_index:
                    # If not, create a new task index
                ...
                # Update the task index in the task
            ...
            for episode in episodes:
                ...
                if old_index in stats_map:
                    ...
                    if "stats" in stats and folder_dimensions[folder] < max_dim: 
            ...
            # Update all_tasks with unique tasks from this folder
    ...
    if "features" in info:
        ...
        actual_max_dim = max_dim # Use the provided max_dim as the default
    ...
    print(f"total videos: {total_videos} (Update total videos to: {total_videos})")
...
def fill_dataset(source_folders, dataset_name):
    data_dict = {}

    """
    (Fill dataset with data and metadata)
    """
    # (Set up argument parser)
    parser = argparse.ArgumentParser(description="Fill dataset with data and metadata")
    ...
    merge_datasets(args.source, args.output, max_dim=args.max_dim, default_fps=args.fps)
...
if __name__ == "__main__":
    ...
    #episode_path = "/home/rlab/.cache/huggingface/lerobot/bjb7/so101_pen_touch_test_1"
